        ...
        for line in in_file:
            ...
            for i in range(steps_list[i1] // 2 - 1):
                if value < cut[i1][i]:
                    irange = i
                    break
            # If dist is too big, clasiffy into rightmost bin.
            if irange < 0 and value < 7.66:
                irange = steps_list[i1] // 2 - 1

            if irange >= 0:
                statis[i0][i1][i2][irange] += 1
                res_nrec[i1] += 1 # Number of records for resi i1
...
def perform_bayesian_statistics(statis, res_nrec, steps_list, map_list):
    for c in range(len(map_list)):
        ...
        if res_nrec[c] > 0:
            ...
            # Smoothing prob of statis[p][c][n]
            for p in range(len(map_list)):
                for n in range(len(map_list)):
                    ...
                    if tmp_sum == 0:
                        ...
                        nnn = 0
                        # Add all i-1,i overlapping sequence probability
                        ...
                        if nnn > 0:
                            tmp_ns /= nnn
                        nnn = 0
                        # Add all i,i+1 overlapping sequence probability
                        ...
                        if nnn > 0:
                            tmp_ps /= nnn
                        # P(i-1,i,i+1) ~= Sum(x, P(i-1,i,x)) * Sum(x, P(x,i,i+1))
            ...
            for i in range(20): # p
                for j in range(20): # n
                    for k in range(states): # Type of SS
                        B[index] = PN_S[i][j][k] # probability of triplet sequence (p, *c, n) given SS 
                        p_s = i * states + k
            ...
            # Read the SVD data
            PSI_AA = np.zeros((20, states)) # Prior: P( i-1 | i, i+1, state)
            PHI_AA = np.zeros((20, states)) # Prior: P( i+1 | i-1, i, state)
            ...
            expectation = np.zeros((20, 20, states))
            expectation_counts = np.zeros((20, 20, states))

            for i in range(20):
                for j in range(20):
                    for iaa in range(states):
                        # Joint prior prob P(i-1, i, i+1) = PSI * PHI
                    ...
                    scale = OCCUP / A0
                    alpha[i][j] *= scale
                    expectation[i][j] = (alpha[i][j] + statis[i][c][j][:states] + 1) / (OCCUP + Y0 + states)
                    # With NO observation, P(triplet, alpha) = P(triplet, beta) = P(triplet, PPII) = 1/3 or 1/2
                    Y0 = max(Y0, 1)
                    expectation_counts[i][j] = Y0 * (alpha[i][j] + statis[i][c][j][:states] + 1) / (OCCUP + Y0 + states)
                    # Scaled counts
            # Output posterior data
            for i in range(20):
                for j in range(20):
                    for k in range(states):
                        print(f"{map_list[i]} {map_list[c]} {map_list[j]} {k:4d}: {statis[i][c][j][k]:4d} {expectation_counts[i][j][k]:8.3f} {expectation[i][j][k]:8.3f}")
